...
from numpy import repeat, sort, unique
...
class FreeClimber:
    ...
    def load_parameters(self):
        ...
        if self.args.debug:
            print('FreeClimber.load_parameters')
    ...
    def read_custom(self, file):
        ...
        print(f'----> {len(lines)} of {len(lines)} specified files have valid paths')
        return _lines

    def get_filelist(self):
        '''Gets the list of files to process. Defaults to all'''
        if self.args.debug:
            print('FreeClimber.get_filelist')
    ...
    def print_new_project(self):
        ...
        if self.args.debug:
            print('FreeClimber.print_new_project')

        print(f"##\n## Analyzing videos in: {self.path_project} \n##" )
        print(f"## Processing {str(len(self.file_list))} videos\n##\n##")
        return
    ...
    def print_new_video(self,name):
        ...
        if self.args.debug:
            print('FreeClimber.print_new_video')

        print('='*72)
        line_to_print = f'== [{self.count} || {len(self.file_list)}] ', self.name
        line_to_print = ('').join(line_to_print)

        if len(line_to_print) <= 80:
    ...
    def concat_slopes(self):
        ...
        if self.args.debug:
            print('FreeClimber.concat_slopes')
    ...
    def create_log_header(self):
        ...
        if self.args.debug:
            print('FreeClimber.create_log_header')
...
#                 else: print('## Files for review @ R = %s' % self.review_R, file = f)
    ...
    def log_video(self, file_name, completed=True): #, review=None):
        ...
        Returns:
          None'''
        if self.args.debug:
            print('FreeClimber.log_video :: completed =',completed)

        ## Naming a variable
        if completed:
            path = 'log/completed.log'
        else:
            path = 'log/skipped.log'
...
#         elif review != None: path = '/review_at_R_%s/review.log' % self.review_R
    ...
    def print_closing(self):
        ...
        if self.args.debug:
            print('FreeClimber.print_closing')

        print('\nVideo processing complete!\n')
        print(f'Total videos: {len(self.file_list)}')
...
def startup():
    ...
    def print_line(line,line_length):
        ...
        if len(line) <= line_length:
            string = line + '#'*(line_length-len(line))
        else:
            string = line
        print(string)
        return
    ...
    lines = [
        '#'*line_length,
        f'## FreeClimber v.{str(version)} ',
        f'## Please cite: {doi}',
        f"## Beginning program @ {str(now)} ",
        '#'*line_length,
    ]
